...
static uint8_t service_uuid128[] = { 0x6e, 0x40, 0x00, 0x01, 0xb5, 0xa3, 0xf3, 0x93, 0xe0, 0xa9, 0xe5, 0x0e, 0x24, 0xdc, 0xca, 0x9e};
static uint8_t char_rx_uuid128[] = { 0x6e, 0x40, 0x00, 0x02, 0xb5, 0xa3, 0xf3, 0x93, 0xe0, 0xa9, 0xe5, 0x0e, 0x24, 0xdc, 0xca, 0x9e};
static uint8_t char_tx_uuid128[] = { 0x6e, 0x40, 0x00, 0x03, 0xb5, 0xa3, 0xf3, 0x93, 0xe0, 0xa9, 0xe5, 0x0e, 0x24, 0xdc, 0xca, 0x9e};
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        for (uint8_t pos=0;pos<ESP_UUID_LEN_128;pos++) {
			...
			gl_profile_tab[PROFILE_A_APP_ID].service_id.id.uuid.uuid.uuid128[pos] = service_uuid128[ESP_UUID_LEN_128 - pos - 1];
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        rsp.attr_value.len = 6;         // Jacky - send data
        rsp.attr_value.value[0] = 0x32;
        rsp.attr_value.value[1] = 0x33;
        rsp.attr_value.value[2] = 0x34;
        rsp.attr_value.value[3] = 0x35;
        rsp.attr_value.value[4] = 0x36;
        rsp.attr_value.value[5] = 0x37;
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        for (uint8_t pos=0;pos<ESP_UUID_LEN_128;pos++) {
			// copy correct part of ble_service_uuid128 byte by byte into the service struct
			gl_profile_tab[PROFILE_A_APP_ID].char_uuid.uuid.uuid128[pos] = char_rx_uuid128[ESP_UUID_LEN_128 - pos - 1];
			//ESP_LOGI(GATTS_TAG, "Service %d UUID pos %d: %02x", param->reg.app_id, pos, ble_service_uuid128[pos+16*param->reg.app_id]);
		}

        esp_ble_gatts_start_service(gl_profile_tab[PROFILE_A_APP_ID].service_handle);
        a_property = ESP_GATT_CHAR_PROP_BIT_WRITE | ESP_GATT_CHAR_PROP_BIT_WRITE_NR;
        ...
        for (uint8_t pos=0;pos<ESP_UUID_LEN_128;pos++) {
			// copy correct part of ble_service_uuid128 byte by byte into the service struct
			gl_profile_tab[PROFILE_A_APP_ID].char_uuid.uuid.uuid128[pos] = char_tx_uuid128[ESP_UUID_LEN_128 - pos - 1];
			//ESP_LOGI(GATTS_TAG, "Service %d UUID pos %d: %02x", param->reg.app_id, pos, ble_service_uuid128[pos+16*param->reg.app_id]);
		}
